...
Concurrent publishers coordinate through a persistent `.cohere-transcribe-locks` directory beside the outputs. Its access mode follows the output directory so collaborators who can publish there can use the same locks, and shared writable registries use the sticky bit. A held lock is revalidated at checkpoint and output commit boundaries; replacement of either the output-adjacent lock or the released 0.1.2 compatibility lock aborts the commit. A fully verified `existing: "skip"` run does not need to create or modify the lock directory. The 0.1 release series acquires both lock locations for mixed-version coordination, and the temporary compatibility path is removed automatically in 0.2.0. A filesystem that rejects an exclusive lock on an otherwise read-only lock file fails with a typed runtime error; it never substitutes a non-exclusive lock.
