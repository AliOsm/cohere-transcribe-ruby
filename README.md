...
Concurrent publishers coordinate through a persistent `.cohere-transcribe-locks` directory beside the outputs. Its access mode follows the output directory so collaborators who can publish there can use the same locks; a fully verified `existing: "skip"` run does not need to create or modify the lock directory. Version 0.1.3 also acquires the released 0.1.2 temporary lock path for mixed-version coordination, and that compatibility path is scheduled for removal in 0.2.0.
