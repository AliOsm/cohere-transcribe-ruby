...
- `alignment: "word"` computes full-file emissions with the exact pinned MMS-300M forced-aligner ONNX export, then runs a pure-Ruby float32 CTC Viterbi kernel. Audio shorter than 30 seconds follows the pinned aligner's direct waveform geometry; inputs shorter than its 400-sample receptive field receive only the minimum right padding. An unalignable segment alone falls back to bounded uniform timing without dropping transcript words.
...
Hub artifacts reuse the standard Hugging Face cache. Symbolic branches and tags are revalidated while online with a bounded per-client memo of at most five seconds; concurrent misses refresh independently and each caller uses its own fetched commit. Standard bare-SHA ref reads are capped at 41 bytes, and ref files are atomically replaced as best-effort cache hints without invalidating a valid online result when the cache is unreadable or read-only. Shared-cache directories preserve setgid and execute-gated access; snapshot directories receive a sticky bit where supported, ref directories remain replaceable for atomic collaborator updates, immutable payloads and refs inherit collaborator read access without write access, and download locks inherit the read/write access needed for coordination. Mode updates that the filesystem cannot represent are tolerated, current-user-owned older cache entries are upgraded when possible, readable non-writable locks can still coordinate where the filesystem permits an exclusive lock, and an inaccessible older lock returns a typed error with remediation instead of being replaced while it may still be held. Temporary connection, rate-limit, server, malformed-JSON, or missing-or-invalid-commit responses can reuse a complete cached file snapshot, while authentication and missing-repository responses remain definitive errors. Malformed individual repository-file entries are ignored when valid siblings remain, but an empty or wholly malformed listing is reported directly. Set `HF_HUB_OFFLINE=1` to resolve symbolic revisions only from an already complete cached snapshot; an offline symbolic revision remains at its last successfully cached commit until a later online refresh succeeds. Dense Safetensors and `pytorch_model.bin` weights are streamed into a GGUF conversion once and reused from `~/.cache/cohere-transcribe`. PyTorch metadata is decoded by a restricted, allowlist-only Ruby reader; it never imports Python or executes pickle globals. Current `torch.save` ZIP files and the preceding raw-storage stream format are supported, including sharded indexes and strided tensors. Ancient tar-format checkpoints are rejected because they cannot be interpreted with the same restricted weights-only contract; re-save those as Safetensors or a current state dict. The retained-session identity includes the resolved device and dtype as well as the model identity. Each converted artifact has an independent cache key for its output dtype and a SHA-256 source fingerprint over the model ID/revision, relative source paths, sizes, mtimes, and ctimes, so even a same-size, mtime-preserving rewrite invalidates a local fine-tune through its changed ctime. A sidecar completion marker binds that source fingerprint, output dtype, and cache layout to the converted GGUF's device, inode, size, mtime, and ctime. This marker is stored beside the model as `*.complete.json`. The cache accepts only regular, non-symlink GGUF/marker files; conversion locks use no-follow opens where available and verify that the opened descriptor still matches the path's device and inode. Cross-process output locks use an output-adjacent `.cohere-transcribe-locks/` registry derived from the canonical output set and also acquire the released 0.1.2 temporary lock during this transition.
...
Add a repository Actions secret named `RUBYGEMS_AUTH_TOKEN` containing a RubyGems API key with permission to push `cohere-transcribe`. Publishing a GitHub Release tagged `0.1.3` runs the release workflow, verifies that the tag matches `Cohere::Transcribe::VERSION`, builds the exact `cohere-transcribe-0.1.3.gem` artifact, and pushes it to RubyGems.org. A leading `v` is also accepted by the release check.
